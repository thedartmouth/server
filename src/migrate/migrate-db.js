            ...
            .then((listings) => {
                Promise.all(
                    listings.map((listing) => {
                        return new Promise((resolve, reject) => {
                            Listings.findById(listing._id)
                                .then((toUpdateListing) => {
                                    processListing(toUpdateListing)
                                        .then((updatedListing) => {
                                            modifiedCount += 1
                                            updatedListing
                                                .save()
                    ...
                    .then((modifiedListings) => {
                        console.log(
                            `Modified ${modifiedCount} Listing documents.`
                        )
    ...
    return new Promise((resolve) => {
        mongoose
            ...
            .then(() => {
                ...
                Promise.all([migrateUsers(), migrateListings()])
                    .then(() => {
...
cli.question(
    `Migrating the DB will modify the data connected to ${constants.MONGODB_URI}, are you sure? (Y/N)`,
    async (answer) => {
        cli.close()
        switch (answer) {
            case 'Y':
