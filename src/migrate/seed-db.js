...
const seedUsers = (entries) => {
    return new Promise((resolve, reject) => {
        Promise.all(
            ...
            .then((newUsers) => {
                console.log(
                    `Seeded ${entries.length} new User documents`,
                    newUsers
                )
                resolve(newUsers)
            })
...
const seedListings = (entries) => {
    return new Promise((resolve, reject) => {
        Promise.all(
            ...
            .then((savedListings) => {
                console.log(
                    `Seeded ${entries.length} new Listing documents`,
                    savedListings
                )
        ...
        Listings.find({}).then((listings) => {
            Promise.all(
                listings.map((listing) => {
                    return new Promise((resolve) => {
                        Listings.findById(listing._id).then(
                            (listingToModify) => {
                                listingToModify
                                    .save()
                                    .then((modifiedListing) => {
                ...
                Users.find({}).then((users) => {
                    Promise.all(
                        users.map((user) => {
                            return new Promise((resolve) => {
                                Users.findById(user._id).then(
                                    (userToModify) => {
                                        userToModify.listing =
                                            modifiedListings[0]._id
                                        userToModify
                                            .save()
                                            .then((modifiedUser) => {
...
const seedDB = () => {
    return new Promise((resolve) => {
        ...
        mongoose
            .connect(constants.MONGODB_URI, mongooseOptions)
            .then(() => {
                            ...
                            .then(() => {
                                Promise.all(
                                            ...
                                            (resolve, reject) => {
                                                switch (schemaSet.schema) {
                                                    ...
                                                    default:
                                                        reject(
                                                            new Error(
                                                                'Invalid schema type specified in input data.'
                                                            )
                                                        )
                                                }
                                            }
                                    ...
                                    .then(() => {
                                        ...
                                        linkDocuments().then(() => {
                                            console.log(
                                                'Newly seeded documents linked for testing. Safe to exit.'
                                            )
                                            resolve()
                                        })
                                    })
...
cli.question(
    `Seeding the DB will delete all data connected to ${constants.MONGODB_URI}, are you sure? (Y/N)`,
    async (answer) => {
        cli.close()
        switch (answer) {
            case 'Y':
