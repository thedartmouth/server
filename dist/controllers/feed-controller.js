...
function selectionMerge(arraysToMerge) {
  ...
  const sortedArray = []; // use a set to make sure that articles are unique

  const UUIDSet = new Set(); // calculate number of items => number of iterations

  const numItems = arraysToMerge.reduce((val, subArr) => {
    return val + subArr.items.length;
  }, 0); // select the most recent article from the 4 arrays as queues

  for (let i = 0; i < numItems; i += 1) {
    // extract the most recent item from the arrays
    const lowestIdx = arraysToMerge.reduce((lowIdx, subArr, currIdx, bigArr) => {
      // nth list is empty
      if (!subArr.items[0]) {
        return lowIdx;
      } else if (!bigArr[lowIdx].items[0] || subArr.items[0].created_at > bigArr[lowIdx].items[0].created_at) {
        return currIdx;
      } else {
        return lowIdx;
      }
    }, 0); // remove the item from the right array

    const lowestItem = arraysToMerge[lowestIdx].items.shift();
    const {
      uuid
    } = lowestItem; // check uniqueness constraint against the set

    if (!UUIDSet.has(uuid)) {
      UUIDSet.add(uuid); // add it to the compiled list if UUID is new
...
async function fetchParallel(array, AuthorsOrTags) {
  ...
  try {
    ...
    const alldata = await Promise.all(array.map(async ({
      name
    }) => {
      // encode http spaces
      name.replace(' ', '+');
      const resp = await _axios.default.get(_fetchURL.default[AuthorsOrTags] + name);
      return resp.data;
...
async function fetchFollowingFeed(user, AuthorsOrTags) {
  ...
  try {
    const populatedUser = await user.populate(`followed${AuthorsOrTags}`, 'name').execPopulate(); // console.log(populatedUser);

    const following = populatedUser[`followed${AuthorsOrTags}`]; // console.log(following);

    const output = await fetchParallel(following, AuthorsOrTags);
    return (0, _cleanArticles.default)(output);
  } catch (error) {
    throw error;
  }
}
